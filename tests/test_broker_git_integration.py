...
class GitProvisioning(unittest.TestCase):
 ...
 def test_real_git_checkout_is_idempotent_and_server_composed(self):
  m=self.manifest();first=self.b.provision(m,self.auth(),'jwt');second=self.b.provision(m,self.auth(),'jwt');self.assertTrue(first.resolve().is_relative_to((self.root/'workloads').resolve()));self.assertTrue(any((self.root/'repos').iterdir()));self.assertTrue(self.chown.called);self.assertEqual(first,second);self.assertEqual((first/'README.md').read_text(),'source\n');self.assertEqual((first/'AGENTS.md').read_text(),'server\n');self.assertTrue((first/'.git').exists());self.assertFalse((first/'.git/objects/info/alternates').exists());self.assertEqual(subprocess.check_output(['git','-c',f'safe.directory={first}','-C',first,'branch','--show-current'],text=True).strip(),'task/33333333/wip')
 ...
 def test_privileged_push_never_executes_checkout_hook(self):
  m=self.manifest();work=self.b.provision(m,self.auth(),'jwt');hook=work/'.git/hooks/pre-push';marker=self.root/'hook-ran'
  hook.write_text(f'#!/bin/sh\ntouch {marker}\nexit 91\n');hook.chmod(0o755)
  self.b._publish({'id':'wo','project':'alpha','worktree':str(work),'manifest':m})
